...
import {
  ...
  ErrorType,
} from "../../../src/service/RouteOptimization";
import {PossiblePathes} from "../../../src/types/PossiblePathes";
import {
  IOptimizationStrategy,
}
  from "../../../src/service/strategy/IOptimizationStrategy";
...
describe("RouteOptimization", () => {
  ...
  const strategyMock: IOptimizationStrategy = {
    optimize(_pathes: PossiblePathes, _from: string, _to: string): Promise<PathTime> {
  ...
  describe("optimize", () => {
    it("should throw an error if the optimization strategy is not defined", () => {
      expect(() => routeOptimization.optimize("A1", "A7")).toThrow(
